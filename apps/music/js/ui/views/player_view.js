    ...
    musicdb.getFile(songData.name, function(file) {
      AlbumArtCache.getCoverBlob(songData).then(function(pictureBlob) {
        ...
        if (PlayerView.playStatus !== PLAYSTATUS_PLAYING) {
          ...
          // When the activity is done, we stop competing.
          // And if we have been paused, then we resume.
          a.onerror = a.onsuccess = function() {
            PlayerView.playStatus === PLAYSTATUS_PAUSED && PlayerView.play();
          };
        }
      });
    });
